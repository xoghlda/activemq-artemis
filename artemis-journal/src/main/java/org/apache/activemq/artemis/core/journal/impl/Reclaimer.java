   ...
   public static void scan(final JournalFile[] files) {
      for (int i = 0; i < files.length; i++) {
         ...
         if (!currentFile.isNegReclaimCriteria()) {
            ...
            for (int j = i - 1; j >= 0 && !outstandingNeg; j--) {
               ...
               if (!file.isCanReclaim() && currentFile.getNegCount(file) != 0) {
                  logger.trace("%s can't be reclaimed because %s has negative values", currentFile, file);
                  outstandingNeg = true;
               }
            }
         ...
         if (!currentFile.isPosReclaimCriteria()) {
            ...
            logger.trace("posCount on %s = %d", currentFile, posCount);
            ...
            for (int j = i; j < files.length && negCount < posCount; j++) {
               ...
               if (logger.isTraceEnabled() && toNeg != 0) {
                  logger.trace("Negative from %s into %s = %d", files[j], currentFile, toNeg);
               }
            }

            if (negCount < posCount) {
               logger.trace("%s can't be reclaimed because there are not enough negatives %d", currentFile, negCount);
            } else {
               logger.trace("%s can be reclaimed", currentFile);
