      ...
      try (PreparedStatement currentDateTime = connection.prepareStatement(currentDateTimeSql)) {
         ...
         try (ResultSet resultSet = currentDateTime.executeQuery()) {
            ...
            if (currentTimeNoMillis < startTime) {
               logger.warn("currentTimestamp = {} on database should happen AFTER {} on broker", currentTimeNoMillis, startTime);
            }
            if (currentTimeNoMillis > endTime) {
               logger.warn("currentTimestamp = {} on database should happen BEFORE {} on broker", currentTimeNoMillis, endTime);
            }
            return currentTime;
         }
      }
